...
model = AutoModelForCTC.from_pretrained(model_id, ignore_mismatched_sizes=True)

# Set the target language to Dutch
...
def save_to_excel(data_records, iteration, group_name):
    ...
    output_path = os.path.join(output_dir, f"{group_name}_MMS_{iteration}.xlsx")
    df.to_excel(output_path, index=False)
    print(f"Data saved to Excel file at: {output_path}")
    return output_path

# Initialize log file
log_file_path = os.path.join(base_path, "processing_log_MMS.txt")
...
for text_file_path in text_file_paths:
    ...
    for idx, line in enumerate(lines, 1):  # Start index from 1
        if idx % 100 == 0:
            ...
            try:
                print("Cumulative WER: ", jiwer.wer(all_ground_truths, all_transcriptions))
            except:
                print("Error calculating WER")
        ...
        try:
            ...
            # Buffer the audio and transcription
            audio_buffer.append(audio_segment)
            transcription_buffer.append(ground_truth)

            # Process in chunks of 25 audio files
            if len(audio_buffer) == 25:
                combined_audio = np.concatenate(audio_buffer)
                combined_transcription = ' '.join(transcription_buffer)

                # Process the combined audio input
                inputs = processor(combined_audio, sampling_rate=16000, return_tensors="pt", padding=True).input_values.to(device)
                ...
                ground_truth_transformed = transformation(combined_transcription)
                ...
                if not ground_truth_transformed.strip() or not transcription_transformed.strip():
                    print(f"Skipping empty transformation result for index {idx}")
                    # Clear the buffers
                    audio_buffer = []
                    transcription_buffer = []
                    continue
